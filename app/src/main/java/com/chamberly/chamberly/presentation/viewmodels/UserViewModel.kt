...
//        }
    ...
    private fun showToast(message: String) {
     ...
     fun checkAndUpdateCoins(userId: String, onComplete: (Boolean) -> Unit) {
        val userDocRef = firestore.collection("Accounts").document(userId)

        userDocRef.get().addOnSuccessListener { document ->
            if (document != null) {
                val coins = document.getLong("Coins") ?: 0L
                if (coins > 0) {
                    userDocRef.update("Coins", coins - 1).addOnSuccessListener {
                        onComplete(true)
                    }.addOnFailureListener {
                        onComplete(false)
                    }
