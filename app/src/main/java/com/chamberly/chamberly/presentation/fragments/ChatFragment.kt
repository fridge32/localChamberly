...
class ChatFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val gameButton = view.findViewById<Button>(R.id.buttonGame)
...
//        window?.attributes = layoutParams
    ...
    private fun showQuestionCard(response: String){
        chamberViewModel.sendQuestionCard(
            UID = userViewModel.userState.value!!.UID,
    ...
    private fun showGamePopup(){
        ...
        val popupView = inflater.inflate(R.layout.popup_choose_game, null)

        val alertDialog = AlertDialog.Builder(requireContext())
            .setView(popupView)
            .create()

        popupView.findViewById<Button>(R.id.button_random_questions).setOnClickListener {
            val userId = FirebaseAuth.getInstance().currentUser?.uid
            if (userId != null) {
                userViewModel.checkAndUpdateCoins(userId) { success ->
                    if(success){
                        chamberViewModel.getChamberName(
                            userViewModel.chamberID.value ?: "",
                            onSuccess = { chamberTitle ->
                                sendOpenAIRequest(chamberTitle) { response->
                                    showQuestionCard(response)
                                }
        ...
        popupView.findViewById<Button>(R.id.button_pick_topic).setOnClickListener {
            alertDialog.dismiss()
            showPickQuestionsPopup()
        }
    ...
    private fun showPickQuestionsPopup(){
        val inflater = layoutInflater
        val popupView = inflater.inflate(R.layout.popup_pick_question, null)
        val editTextTopic = popupView.findViewById<EditText>(R.id.editTextTopic)
        ...
        popupView.findViewById<Button>(R.id.buttonGenerateQuestion).setOnClickListener {
            ...
            if (userId != null) {
                userViewModel.checkAndUpdateCoins(userId) { success ->
                    if(success){
                          if (topic.isNotEmpty()) {
                                sendOpenAIRequest(topic) { response ->
                                showQuestionCard(response)
                                }
                          }
    ...
    private fun sendOpenAIRequest(topic: String, callback: (String) -> Unit) {
        val apiKey = ""
        val okHttpHandler = OkHttpHandler(requireContext(), "", JSONObject(), JSONObject())

        okHttpHandler.postGPTRequest(apiKey, topic, object : Callback {
            override fun onFailure(call: okhttp3.Call, e: IOException) {
                Log.e("OpenAI Request Failure", "Failed to send request", e)
            }

            override fun onResponse(call: okhttp3.Call, response: okhttp3.Response) {
                val responseBody = response.body?.string()
                Log.d("OpenAI Response", responseBody ?: "Empty response body")
                try {
                    ...
                    if (choicesArray.length() > 0) {
                        ...
                        Log.e("OpenAI Response", "No 'choices' array found in the response")
                    }
                } catch (e: JSONException) {
                    Log.e("OpenAI Response", "Error parsing JSON: ${e.message}")
                }
            }
        })
    }
